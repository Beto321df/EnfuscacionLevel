...
for(let i=0;i<4;i+=1){
    ...
    const dispatcherIds = Array.from(dispatcher.matchAll(/\b(?:if|elseif) o==([0-9]+) then/g))
        .map(m => Number(m[1]));
    ...
    assert(!/\bif o==[0-9]+ then\s*(?:elseif|else error)/.test(out),'X7.1 output no debe contener handlers vacíos');
