...
function alphabets(compact = false) {
    if (compact) {
        return [shuffle(Array.from('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_')).join(''), ''];
    }
    return [
...
function encodePayload(bytes, seed, step, hi, lo, rolling = false, compact = false) {
    const chunks = [];
    let part = '';
    let k1 = seed & 255;
    ...
    for (let i = 0; i < bytes.length; i += 3) {
        let aByte, bByte, cByte;
        for (let laneIndex = 0; laneIndex < 3; laneIndex += 1) {
            const at = i + laneIndex;
            ...
            if (laneIndex === 0) aByte = v;
            else if (laneIndex === 1) bByte = v;
            else cByte = v;
        }
        const aVal = aByte || 0;
        const bVal = bByte || 0;
        const cVal = cByte || 0;
        part += hi[(aVal >> 2) & 63];
        part += hi[((aVal & 3) << 4) | ((bVal >> 4) & 15)];
        part += bByte === undefined ? '=' : hi[((bVal & 15) << 2) | ((cVal >> 6) & 3)];
        part += cByte === undefined ? '=' : hi[cVal & 63];
        if (part.length >= 8192) {
            chunks.push(part);
            part = '';
        }
