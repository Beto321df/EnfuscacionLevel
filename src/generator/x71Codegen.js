...
function encodePayload(bytes, seed, step, alphabet, rolling = false) {
    ...
    const emit12 = value => alphabet[value % 92] + alphabet[Math.floor(value / 92)];
    for (let i = 0; i < bytes.length; i += 3) {
        ...
        const u = enc[0] * 16 + Math.floor(enc[1] / 16);
        const v = (enc[1] % 16) * 256 + enc[2];
        part += emit12(u) + emit12(v);
        if (part.length >= 8192) {
            chunks.push(part);
            part = '';
        }
