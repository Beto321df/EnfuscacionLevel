...
function stabilizeLuaTokenBoundaries(source) {
    ...
    return String(source)
        .replace(/\bthen(?=local|return|if|for|while|repeat|do)/g, 'then;')
        .replace(/\belse(?=local|return|if|for|while|repeat|do)/g, 'else;')
        .replace(/\bdo(?=local|return|if|for|while|repeat)/g, 'do;');
}
