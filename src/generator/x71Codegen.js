...
function pruneDispatcherHandlers(source, used) {
    ...
    const branchRe = /(?:^| )(?:(?:if)|(?:elseif)) o==([0-9]+) then/g;
    const branches = [];
    let match;
    while ((match = branchRe.exec(chain)) !== null) {
        const bodyStart = match.index + match[0].lastIndexOf(match[0].trim().startsWith('elseif') ? 'elseif' : 'if');
        branches.push({
            id: Number(match[1]),
            start: bodyStart,
    ...
    const rebuilt = kept.map((branch, i) => {
        ...
        // clause.
        const body = branch.body || (branch.id === 1 ? 'pc=pc' : 'pc=pc');
...
function mangleRuntimeIdentifiers(source) {
    ...
    const names = [
        'UNPACK','INVK','D32','D16','CINV','LINV','INV',
